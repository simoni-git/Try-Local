...
class FastivalDetailVC: UIViewController {
    ...
    private func configureUI() {
        ...
        for label in [periodLabel, adressLabel, hostLabel, callNumLabel, linkLabel] {
            label?.font = UIFont(name: "PretendardGOV-SemiBold", size: 14)
        }
        
        titleLabel.text = self.vm.name
        explanationLabel.text = self.vm.descriptionText
        periodLabel.text = self.vm.period
        adressLabel.text = self.vm.address
        hostLabel.text = self.vm.organizer
...
extension FastivalDetailVC: UICollectionViewDataSource , UICollectionViewDelegate, UICollectionViewDelegateFlowLayout{
...
extension FastivalDetailVC: UIAdaptivePresentationControllerDelegate {
