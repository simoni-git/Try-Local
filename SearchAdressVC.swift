...
protocol sendStartAdressDelegate {
    func sendStartAdress(adress: String)
}
protocol sendRestPointAdressDelegate {
    func sendRestPointAdress(adress: String)
}
class SearchAdressVC: UIViewController {
    ...
    var vm: SearchAdressVM!
    var startAdressDelegate: sendStartAdressDelegate?
    var restPointAdressDelegate: sendRestPointAdressDelegate?
...
//MARK: - TabelView 관련
extension SearchAdressVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let selectedAdress = vm.addressList[indexPath.row]
        let rodeAdress = selectedAdress.roadAddr
        print("선택된 도로명 주소: \(rodeAdress)")
        if vm.isStartAdressMode {
            startAdressDelegate?.sendStartAdress(adress: rodeAdress)
        } else {
            restPointAdressDelegate?.sendRestPointAdress(adress: rodeAdress)
        }
